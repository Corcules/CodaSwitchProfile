...
-(void)switchToProfile:(NSString*)targetProfil deviceId:(NSString*)deviceId
{
    if(targetProfil != nil) {
        
        NSDictionary *json = nil;
        json = @{
                 @kESDSDKCommonEvent: @kESDSDKEventSwitchToProfile,
                 @kESDSDKCommonContext: self.pluginUUID ,
                 @kESDSDKCommonDevice: deviceId ,
                 @kESDSDKCommonPayload: @{
                         @kESDSDKPayloadProfile:targetProfil
                         }
                 };
        ...
        if (err == nil)
        {
            ...
            if(error != nil)
            {
                NSLog(@"CORCULES SWITCH PROFILE ERROR SEND SOCKET error %@", error);
            }
        }
